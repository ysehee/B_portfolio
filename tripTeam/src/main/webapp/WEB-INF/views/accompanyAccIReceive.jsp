...
<head>
	...
	<title>Blue Horizon 블루호라이즌 마이페이지 - 받은 동행 신청</title>
...
<div id="wrap">
	...
	<div class="accompany_box">
		<div class="accompany_title">
			<h1 class="font_800_accom" >내가 받은 동행신청</h1>
		...
		<table>
			...
			<tbody>
			<c:forEach var="accompanyReceivePickList" items="${accompanyReceivePickList}">
				<tr class="boardBody">
					<td class="title">
						<a href="<c:url value="/accompany/read?accompanyNo=${accompanyReceivePickList.accompanyNo}"/>">${accompanyReceivePickList.accompanyTitle}</a>
					</td>
					<td class="writer"> ${loginUserId}</td>

					<td class="request_id">${accompanyReceivePickList.accompanyPickApyUser}</td>
					<td class="requset_msg">${accompanyReceivePickList.accompanyMessage}</td>
					<td class="company_existence_button">
						<button type="button" onclick="accPickYesBtn(this)" value="1">수락</button>
						<button type="button" onclick="accPickNoBtn(this)" value="2">거절</button>
					</td>

						<%--수락/거절 눌렀을 때 DB 에 저장하기 위한 데이터--%>
					<input type="hidden" class="accompanyNo" value="${accompanyReceivePickList.accompanyNo}">
					<input type="hidden" class="accompanyTitle" value="${accompanyReceivePickList.accompanyTitle}">
					<input type="hidden" class="accompanyWriter" value="${accompanyReceivePickList.accompanyWriter}">
					<input type="hidden" class="accompanyPickApyUser" value="${accompanyReceivePickList.accompanyPickApyUser}">
					<input type="hidden" class="accompanyPickNo" value="${accompanyReceivePickList.accompanyPickNo}">
						<%-------------------------------------------%>
...
<script>
	...
	// 동행 신청 수락
	let accPickYesBtn = function (e) {
		console.log("수락 클릭")

		Rdata = { // 보내줄 값
			accompanyNo: $(e).parent().parent().children('.accompanyNo').val(),
			accompanyTitle: $(e).parent().parent().children('.accompanyTitle').val(),
			accompanyWriter: $(e).parent().parent().children('.accompanyWriter').val(),
			accompanyPickApyUser: $(e).parent().parent().children('.accompanyPickApyUser').val(),
			accompanyYN: 1,

			accompanyPickNo: $(e).parent().parent().children('.accompanyPickNo').val()
		}

		console.log(Rdata);

		$.ajax({
			url: "accPickAnswerBtn",
			type: 'POST',
