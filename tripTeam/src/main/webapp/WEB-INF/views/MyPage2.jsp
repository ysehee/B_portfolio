...
<head>
    ...
    <title>Blue Horizon 블루호라이즌 마이페이지</title>
    ...
    <link rel="stylesheet" href="<c:url value='/css/mypage2.css'/> ">
    ...
    <div class="select_form">
        ...
        <div class="section_left tab-content current" id="tab-1" >
            <div class="form_title3">
                <h1 class="font_800" id="modify_title">예약 내역</h1>
            </div>
            <h2 class="reserve_spot mar_bot font_500b">지난 예약 내역</h2>
            <table>
                <thead>
                <tr>
                    <th class="bno">에약번호</th>
                    <th class="title">예약 상품 이름</th>
                    <th class="start_date">출발일</th>
                    <th class="product_price">상품금액</th>
<%--                    <th class="reserve_people">예약 인원</th>--%>
<%--                    <th class="total_price">결제 금액</th>--%>
                    <th class="payment">결제 방식</th>

                </tr>
                </thead>
                <tbody>
                <c:forEach var="item" items="${prevReservationList}">
                    <tr class="boardBody">
                        <td class="bno"> ${item.packageNo}</td>
                        <td class="title">
                                ${item.packageTheme}
                        </td>
                        <td class="start_date">${item.packageStartDate}</td>

                        <td class="product_price">${item.packagePrice}</td>
                        <td class="payment">신용카드</td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>

            <h2 class="reserve_spot mar_top mar_bot font_500b">현재 예약 내역</h2>
            <table class="mr_b">
                <thead>
                <tr>
                    <th class="bno">에약번호</th>
                    ...
                    <th class="start_date">출발일</th>
                    <th class="product_price">상품금액</th>
                    <th class="payment">결제방식</th>

                </tr>
                </thead>
                <tbody>
                <c:forEach var="item" items="${nextReservationList}">
                    <tr class="boardBody">
                        <td class="bno"> ${item.packageNo}</td>
                        <td class="title">
                                ${item.packageTheme}
