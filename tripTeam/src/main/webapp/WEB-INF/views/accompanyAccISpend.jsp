...
<div id="wrap">
	...
	<div class="accompany_box">
		<div class="accompany_title">
			<h1 class="font_800_accom" >내가 보낸 동행신청</h1>
		...
		<table>
			...
			<tr>
				...
				<th class="reg_date">동행신청 일자</th>
				<th class="company_existence">동행 신청 취소 여부</th>

			</tr>
			</thead>
			<tbody>
			<c:forEach var="accompanySpendPickList" items="${accompanySpendPickList}">
				<tr class="boardBody">
					<td class="title">
						<a href="<c:url value="/accompany/read?accompanyNo=${accompanySpendPickList.accompanyNo}"/>">${accompanySpendPickList.accompanyTitle}</a>
					</td>
					<td class="writer"> ${accompanySpendPickList.accompanyWriter}</td>

					<td class="requset_msg">${accompanySpendPickList.accompanyMessage}</td>

					<td class="reg_date"><fmt:formatDate value="${accompanySpendPickList.accompanyApyDate}" pattern="yyyy-MM-dd" type="date"/></td>

					<td class="company_existence">
						<input type="hidden" class="accompanyPickNo" name="accompanyPickNo" value=${accompanySpendPickList.accompanyPickNo}>
						<button type="button" id="accPickSpendCancelBtn" onclick="accPickSpendCancelBtn(this)" class="a_btn">취소</button>
					</td>
...
<script>
	...
	// 동행 신청 거절
	let accPickSpendCancelBtn = function (e) {
		console.log("신청 취소 클릭")

		Rdata = { // 보내줄 값
			accompanyPickNo: $(e).parent().children('.accompanyPickNo').val()
		}

		console.log(Rdata);

		$.ajax({
			url: "accPickSpendCancel",
			type: 'POST',
